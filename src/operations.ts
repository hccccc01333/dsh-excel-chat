...
function absoluteColumnRef(sheetName: string, colLetter: string, fromRow: number, toRow: number): string {
  ...
  for (const [index, operation] of operations.entries()) {
    switch (operation.op) {
      ...
      case 'hideColumns': {
        const sheet = findSheet(workbook, operation.sheet)
        if (!sheet) throw new Error(`sheet not found: ${operation.sheet}`)
        if (!operation.columns.length) throw new Error('hideColumns requires at least one column')
        const hidden = operation.hidden ?? true
      ...
      case 'groupColumns': {
        ...
        const level = operation.level ?? 1
        if (from < 1 || to < from) throw new Error(`invalid groupColumns: from=${operation.from} to=${operation.to}`)
        for (let col = from; col <= to; col++) {
          const target = sheet.getColumn(col)
          target.outlineLevel = level
          if (operation.collapse) target.hidden = true
        }
      ...
      case 'printTitles': {
        const sheet = findSheet(workbook, operation.sheet)
        if (!sheet) throw new Error(`sheet not found: ${operation.sheet}`)
        if (operation.rows) sheet.pageSetup.printTitlesRow = operation.rows
        if (operation.columns) sheet.pageSetup.printTitlesColumn = operation.columns
        break
      }
      case 'copyStyle': {
      ...
      case 'setZoom': {
        const sheet = findSheet(workbook, operation.sheet)
        if (!sheet) throw new Error(`sheet not found: ${operation.sheet}`)
        if (operation.zoom < 10 || operation.zoom > 400) throw new Error(`invalid zoom: ${operation.zoom} (10-400)`)
      ...
      case 'setWorkbookProperties': {
        if (operation.creator !== undefined) workbook.creator = operation.creator
        if (operation.lastModifiedBy !== undefined) workbook.lastModifiedBy = operation.lastModifiedBy
        if (operation.title !== undefined) workbook.title = operation.title
        if (operation.subject !== undefined) workbook.subject = operation.subject
        if (operation.description !== undefined) workbook.description = operation.description
        if (operation.keywords !== undefined) workbook.keywords = operation.keywords
        if (operation.recalcOnOpen) workbook.calcProperties.fullCalcOnLoad = true
        break
      }
      case 'rankColumn': {
      ...
      case 'addSparklines': {
        ...
        if (dataBang < 0 || locBang < 0) throw new Error('addSparklines dataRange and locationRange must be sheet-qualified')
        const sparkSheet = findSheet(workbook, operation.dataRange.slice(0, dataBang))
        if (!sparkSheet) throw new Error(`sheet not found: ${operation.dataRange.slice(0, dataBang)}`)
        const qualified = (range: string, bang: number): string =>
          `${qualifySheetName(sparkSheet.name)}!${range.slice(bang + 1)}`
...
function displayTextOf(cell: ExcelJS.Cell): string {
...
function clearRange(workbook: ExcelJS.Workbook, range: string, mode: 'contents' | 'formats' | 'all'): void {
  const parsed = parseRange(workbook, range)
  for (let row = parsed.startRow; row <= parsed.endRow; row++) {
    for (let col = parsed.startCol; col <= parsed.endCol; col++) {
      const cell = parsed.sheet.getCell(`${numberToColumn(col)}${row}`)
...
function copyStyle(workbook: ExcelJS.Workbook, sourceId: string, targetRange: string): void {
  const source = resolveCell(workbook, sourceId)
...
function uniqueValues(workbook: ExcelJS.Workbook, options: Extract<ExcelOperation, { op: 'uniqueValues' }>): number {
  const parsed = parseRange(workbook, options.source)
  const target = parseTargetCell(workbook, options.target, parsed.sheet.name)
  const seen = new Set<string>()
...
function applySheetView(sheet: ExcelJS.Worksheet, patch: (view: ExcelJS.WorksheetView) => void): void {
...
function moveSheet(workbook: ExcelJS.Workbook, name: string, position: number): void {
...
function applyRankColumn(workbook: ExcelJS.Workbook, options: Extract<ExcelOperation, { op: 'rankColumn' }>): void {
  const parsed = parseRange(workbook, options.range)
