 ...
 */
export async function profileWorkbook(path: string, sheet?: string): Promise<WorkbookProfile> {
  const workbook = new ExcelJS.Workbook()
  await workbook.xlsx.load(stripPivotTableParts(await readFile(path)) as any)
  const sheets: SheetProfile[] = []
  for (const worksheet of workbook.worksheets) {
    if (sheet && worksheet.name.toLowerCase() !== sheet.toLowerCase()) continue
...
function profileSheet(worksheet: ExcelJS.Worksheet): SheetProfile {
  ...
  let maxCol = 0
  let formulaCells = 0
  const rows: Array<Map<number, ExcelJS.Cell>> = []
  worksheet.eachRow({ includeEmpty: false }, (row) => {
    const cells = new Map<number, ExcelJS.Cell>()
    row.eachCell({ includeEmpty: false }, (cell) => {
      ...
      const rowNumber = cell.row as unknown as number
      cells.set(col, cell)
...
function detectHeaderRow(rows: Array<Map<number, ExcelJS.Cell>>): number | null {
  for (let i = 0; i < Math.min(rows.length, 20); i++) {
    let textCells = 0
    for (const cell of rows[i]!.values()) {
      if (cell.formula) continue
      const text = cellValueText(cell)
      if (text && !/^[+-]?[\d.,%]+$/.test(text)) textCells++
    }
    if (textCells >= 2) return i + 1
  }
  return null
}
...
function profileColumn(
  ...
  rows: Array<Map<number, ExcelJS.Cell>>,
  col: number,
  headerRow: number | null,
  dataStart: number,
  profiledRows: number,
...
function cellValue(cell: ExcelJS.Cell): unknown {
  if (cell.formula) return (cell.result ?? cellValueText(cell)) || null
...
function cellValueText(cell: ExcelJS.Cell): string {
