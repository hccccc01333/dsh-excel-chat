...
export function annotateWorkbookXml(data: Uint8Array, annotations: WorkbookAnnotations, sheetFileOf: Map<string, string>): Uint8Array {
  ...
  for (const match of workbookXml.matchAll(/<sheet[^>]*name="([^"]*)"[^>]*r:id="([^"]+)"/g)) {
    ...
    if (target) sheetFileOf.set(name, `xl/${target.replace(/^\/?xl\//, '').replace(/^\//, '')}`)
  }
  ...
  for (const [sheetName, comments] of annotations.comments) {
    ...
    if (!/<Default Extension="vml"/.test(contentTypes)) {
      contentTypes = contentTypes.replace(
        /(<Types[^>]*>)/,
        '$1<Default Extension="vml" ContentType="application/vnd.openxmlformats-officedocument.vml"/>',
      )
    }
    vmlFileIndex += 1
...
function commentsXml(author: string, comments: CommentSpec[]): string {
  const items = comments.map((comment) =>
    `<comment ref="${escapeXml(comment.ref)}" authorId="0"><text><r><t xml:space="preserve">${escapeXml(comment.text)}</t></r></text></comment>`,
  ).join('')
