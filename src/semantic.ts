...
import { profileWorkbook, type ColumnProfile, type SheetProfile } from './profile.ts'
...
export interface SemanticProfile {
  ...
  joinKeys: Array<{ left: string; right: string; key: string }>
  summary: string
}
...
function columnRole(profile: ColumnProfile): ColumnRole {
  const header = profile.header ?? ''
  if (profile.dtype === 'date' || TIME_PATTERN.test(header)) return 'time'
  if (ID_PATTERN.test(header)) return 'id'
  if (MEASURE_PATTERN.test(header) || profile.dtype === 'number') return 'measure'
  if (profile.dtype === 'string' && header !== '') return 'dimension'
  return 'unknown'
}
 ...
 */
export async function buildWorkbookSemanticProfile(path: string, sheet?: string): Promise<SemanticProfile> {
  const profile = await profileWorkbook(path, sheet)
  const cells = await readWorkbookCells(await readFile(path))
  const formulasBySheet = new Map<string, string[]>()
  ...
  const sheets: SemanticSheet[] = profile.sheets.map((sheetProfile: SheetProfile) => {
  ...
  const joinKeys: Array<{ left: string; right: string; key: string }> = []
  ...
  const summary = sheets.map((entry) => {
    ...
    if (entry.derivedMetrics.length > 0) parts.push(`派生=${entry.derivedMetrics.slice(0, 3).join('；')}`)
    return parts.join('；')
  }).join('\n')
