...
function normalizeOperation(operation: ExcelOperation, firstSheet: string): ExcelOperation {
  ...
  if (operation.op === 'style' && raw.style && typeof raw.style === 'object') {
    ...
    if (style.hAlign === undefined && style.horizontal !== undefined) style.hAlign = style.horizontal
    if (style.vAlign === undefined && style.vertical !== undefined) style.vAlign = style.vertical
  }
