 ...
 */
export function detectTableFromCells(cells: Record<string, string>, sheetName?: string): ColumnTable | null {
  const rowsBySheet = new Map<string, Map<number, Array<{ col: string; text: string }>>>()
  for (const [id, content] of Object.entries(cells)) {
    ...
    if (!rowsBySheet.has(sheet)) rowsBySheet.set(sheet, new Map())
