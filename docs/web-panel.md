...
## 大表显示（M6，2026-09-07，issue #3）

行数/列数较多时面板显示不全的修复（`src/client/index.tsx`）：

- **显示上限**：行 200 → 2000，新增列上限 100；超限时在表格下方提示
  “已显示前 2000 行 × 100 列”，不再静默丢数据；
- **全屏**：网格上方新增「全屏」按钮，以 `position:fixed` 覆盖层占满窗口
  （Esc 或按钮退出；窗口缩放自动重建网格，视口高度跟随窗口）；内联视口
  也从 420px 提升到 560px；
- **滚动**：x-data-spreadsheet 内建横/纵滚动条，行×列超过视口即可滚动；
- **宽表编辑修复**：`cell-edited` 的列名换算从 `String.fromCharCode(65 +
  colIndex)`（第 27 列起产出非法引用）改为 Excel 进制换算（AA/BZ/…），
  编辑结果写回的单元格引用与真实列一致。
