...
export function validateCharts(charts, cells) {
    const cellIndex = new Map();
    for (const [id, value] of Object.entries(cells))
        cellIndex.set(id.toUpperCase(), value);
...
function checkDateOrder(range, chart, index, cellIndex, anomalies) {
    if (!range)
        return;
    ...
    if (dates.length < 2 || dates.length !== expandRange(range).length)
        return;
