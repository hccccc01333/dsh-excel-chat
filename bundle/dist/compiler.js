...
function compileNode(ir, base, context) {
    switch (ir.operation) {
        case 'binary':
...
function compileOperand(operand, base, context) {
    switch (operand.kind) {
        case 'column':
...
function compileFilterValue(valueFrom, base, context) {
    const letter = context.table.columns[valueFrom];
    if (letter)
        return `${letter}${base.row}`;
    if (/^[A-Za-z]{1,3}[0-9]+$/.test(valueFrom))
        return valueFrom;
    if (/^[+-]?[0-9.]+$/.test(valueFrom))
        return valueFrom;
    return `"${valueFrom}"`;
}
function columnLetter(name, context) {
...
function assertIr(ir) {
    ...
    switch (ir.operation) {
        case 'binary': {
            if (!BINARY_OPERATORS.has(ir.operator))
                throw new Error(`unsupported binary operator: ${ir.operator}`);
            assertOperand(ir.left);
            assertOperand(ir.right);
            return;
        }
        case 'aggregate': {
            if (!AGGREGATE_FUNCTIONS.has(ir.function))
                throw new Error(`unsupported aggregate function: ${ir.function}`);
            if (typeof ir.metric !== 'string' || ir.metric.length === 0)
                throw new Error('aggregate IR requires metric');
            if (!Array.isArray(ir.filters))
                throw new Error('aggregate IR requires filters array');
            return;
        }
        case 'ratio':
...
function assertOperand(operand) {
    ...
    switch (operand.kind) {
        case 'column':
            if (typeof operand.column !== 'string' || operand.column.length === 0)
                throw new Error('column operand requires column');
            return;
        case 'cell':
        ...
        case 'constant':
            if (typeof operand.value !== 'number' || !Number.isFinite(operand.value))
                throw new Error('constant operand requires finite number');
            return;
        default:
